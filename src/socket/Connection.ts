...
const connection = (server: Server): void => {
  ...
  io.attach(server).on('connection', (socket: socketIo.Socket) => {
    ...
    socket.on(
      ...
      async (contactEncrypted, timestamp = 1000): Promise<void | boolean> => {
        ...
        const [publicKeyDestination] = await new Promise<string[]>(resolve => {
          Redis.hmget(`Users:${contact.user}:data`, 'publicKey', (err, data) =>
            resolve(data)
          );
        });
        ...
        Redis.hmset(`Contact:add:${contact.user}:per:${socketQuery.userName}`, {
          contact: newContact,
